...
//        }
        ...
        if (StringUtil.isNullOrEmpty(dto.getLeaveStartTime()) || StringUtil.isNullOrEmpty(dto.getLeaveEndTime())) {
            return ResponseBean.responseError("时间不能为空");
        }
        return null;
    }
        ...
        if(flag){
            ...
            if (!dto.getAuditPerson().equals(oldAuditPerson)) {//如果修改的时候，改变了社会人。则发送任务
              ...
              //  this.myTaskService.saveMyTask(entity.getId(), myTaskType, companyId, dto.getAuditPerson(),true,userId,companyId);
                ...
                expMainService.sendMessageForAudit(entity.getId(),companyId, dto.getAuditPerson(),entity.getType(),dto.getAccountId(),expAuditEntity.getId(),"0");//提交
            }
        }
