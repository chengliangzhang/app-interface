...
public class MyTaskServiceImpl extends GenericService<MyTaskEntity> implements MyTaskService {
    ...
    public List<MyTaskListDTO> getMyTaskList(Map<String, Object> param) throws Exception {
        ...
        map.put("permissionId", "49");//项目费用收款权限
        map.put("companyUserId", param.get("handlerId"));
        map.put("companyId", param.get("appOrgId"));
     ...
     */
    @Override
    public ResponseBean saveMyTask(String targetId, int taskType, String companyId, String companyUserId) throws Exception {
        return saveMyTask(targetId,taskType,companyId,companyUserId,true);
    }

    @Override
    public ResponseBean saveMyTask(String targetId, int taskType, String companyId, String companyUserId,boolean isSendMessage) throws Exception {
        ...
        if(isSendMessage){
            sendMessageForMyTask(taskEntity);
        }
        //推送生产安排的任务
    ...
    private void sendMessageForMyTask( MyTaskEntity taskEntity) throws Exception{
     ...
     */
    @Override
    public ResponseBean saveMyTask(String targetId, int taskType, String companyId) throws Exception {
        ProjectCostPaymentDetailEntity detailEntity = this.projectCostPaymentDetailDao.selectById(targetId);
        if(detailEntity!=null){
                if(taskType == 4 || taskType ==6){
                    this.saveMyTaskFor4Or6(targetId,taskType,companyId,detailEntity.getProjectId());
                }

                if (taskType ==10 || taskType == 20 || taskType == 21){
                    this.saveMyTask(targetId,companyId, taskType,null,detailEntity.getProjectId());
                }
     ...
     * @return:
     */
    private void saveMyTaskFor4Or6(String targetId, int taskType, String companyId,String projectId) throws Exception{
        ...
        if(managerEntity!=null) {
            ...
            this.myTaskDao.insert(taskEntity);
            this.messageService.sendMessage(this.getMessage(taskEntity));
        }
    }
     ...
     * 日期：2017/3/6
     */
    private AjaxMessage saveMyTask(String targetId, String companyId,int taskType,String pointId,String projectId) throws Exception {
        ...
        taskEntity.setParam3(MyTaskRole.FINANCE);
        this.myTaskDao.insert(taskEntity);
        if (!CollectionUtils.isEmpty(companyUserList)) {
    ...
    public ResponseBean saveMyTask(MyTaskEntity entity,boolean isSendMessage) throws Exception {
        ...
        if(isSendMessage) {
            this.sendMessageForMyTask(entity);
        }
        return null;
    }
     ...
     */
    @Override
    public ResponseBean handleMyTask(String targetId, int taskType, String companyUserId) throws Exception {
       ...
       List<MyTaskEntity>  myTaskList = this.myTaskDao.getMyTaskByParam(map);
        //
    ...
    public ResponseBean handleMyTask(String targetId, String companyUserId,String param2) throws Exception {
        ...
        Map<String,Object> map = new  HashMap<String,Object>();
        map.put("targetId",targetId);
        map.put("taskType",SystemParameters.EXP_AUDIT);
        map.put("status","0");
        map.put("handlerId",companyUserId);
        List<MyTaskEntity>  myTaskList = this.myTaskDao.getMyTaskByParam(map);
        //
     ...
     */
    @Override
    public ResponseBean handleMyTask(String id, String result, String status,String accountId) throws Exception {
        ...
        if(null!=myTaskEntity){
            ...
            switch (taskType){
                ...
                case 5:
                case 7:
                    return  handleType5(myTaskEntity,result,accountId);
                case 8:
                case 9:
                    return  handleType8(myTaskEntity,result,status,accountId);
                // case 11:return  handleType11(myTaskEntity,result,accountId);
//                case 12:
//                    return  handleType12(myTaskEntity,status,accountId);
                case 22: //确认签发完成
     ...
     */
    @Override
    public ResponseBean handleMyTask(String id ,String result,String status,String accountId,String paidDate) throws Exception{
        MyTaskEntity myTaskEntity=this.selectById(id);
        if(null!=myTaskEntity){
            int taskType = myTaskEntity.getTaskType();
            switch (taskType){
    ...
    private ResponseBean handleType1( MyTaskEntity myTask,String status,String accountId) throws Exception {
        if(myTask.getTaskType()==3){
            //处理我的任务
            myTask.setStatus("1");
            this.myTaskDao.updateById(myTask);
    ...
    private ResponseBean handleType4( MyTaskEntity myTask,String result,String accountId) throws Exception {
        ...
        if(data!=null){
            ...
            if("2".equals(costEntity.getType())){
                this.saveMyTask((String) data.get("paymentDetailId"),costEntity.getFromCompanyId(),SystemParameters.TECHNICAL_REVIEW_FEE_FOR_PAY,pointId,costEntity.getProjectId());
                this.saveMyTask((String) data.get("paymentDetailId"),costEntity.getToCompanyId(),SystemParameters.TECHNICAL_REVIEW_FEE_FOR_PAID,pointId,costEntity.getProjectId());
            }
            //合作设计费推送任务
            if("3".equals(costEntity.getType())){
                this.saveMyTask((String) data.get("paymentDetailId"),costEntity.getFromCompanyId(),SystemParameters.COOPERATIVE_DESIGN_FEE_FOR_PAY,pointId,costEntity.getProjectId());
                this.saveMyTask((String) data.get("paymentDetailId"),costEntity.getToCompanyId(),SystemParameters.COOPERATIVE_DESIGN_FEE_FOR_PAID,pointId,costEntity.getProjectId());
            }
        }
    ...
    /**
     * 方法描述：企业负责人处理技术审查费（合作设计费）付款确认 type=5，7
     * 作者：MaoSF
     * 日期：2017/1/17
     * @param:
     * @return:
     */
    private ResponseBean handleType5( MyTaskEntity myTask,String result,String accountId) throws Exception {
        //验证身份
        Map<String, Object> map = new HashMap<String, Object>();
        ...
        costOperaterEntity.setCompanyUserId(myTask.getHandlerId());
        costOperaterEntity.setCostDetailId(myTask.getTargetId());
        costOperaterEntity.setType("4");//企业负责人确认类型
        this.projectCostOperaterDao.insert(costOperaterEntity);

        //2.给企业负责人发送付款确认的任务
        ProjectCostPointDetailEntity costDetailEntity = this.projectCostPointDetailDao.selectById(myTask.getTargetId());
        if(costDetailEntity!=null){
          //  costDetailEntity.setField1("1");//代表企业负责人已经确认付款
            this.projectCostPointDetailDao.updateById(costDetailEntity);
        }

        //给对方财务人到款确认的任务
        if(myTask.getTaskType()== SystemParameters.TECHNICAL_REVIEW_FEE_ORG_MANAGER){
            this.sendTaskToPartBFinacial(myTask);
        }
        if(costDetailEntity!=null && myTask.getTaskType()==SystemParameters.COOPERATIVE_DESIGN_FEE_OPERATOR_MANAGER){
            this.sendTaskToCooperativeFinacial(myTask,costDetailEntity);
        }
        //3.处理我的任务
        myTask.setStatus("1");
        this.myTaskDao.updateById(myTask);
    ...
    /**
     * 方法描述：企业负责人处理技术审查费（合作设计费）付款确认 type=8，9,10
     * 作者：MaoSF
     * 日期：2017/1/17
     * @param:
     * @return:
     */
    private ResponseBean handleType8( MyTaskEntity myTask,String result,String status,String accountId) throws Exception {
        //1.验证身份
        Map<String, Object> map = new HashMap<String, Object>();
        map.put("permissionId", "49");//企业负责人权限id
        map.put("companyId", myTask.getCompanyId());
        map.put("userId", accountId);
        List<CompanyUserTableDTO> companyUserList = this.companyUserDao.getCompanyUserByPermissionId(map);
        if(CollectionUtils.isEmpty(companyUserList)){
        ...
        ProjectCostPaymentDetailEntity costDetailEntity = this.projectCostPaymentDetailDao.selectById(myTask.getTargetId());
        if("0".equals(status)){//填写发票信息
//            costDetailEntity.setInvoice(result);
//            this.projectCostPaymentDetailDao.updateById(costDetailEntity);
            return ResponseBean.responseSuccess("操作成功");
        }else {
            //新增记录
            ProjectCostPaymentDetailEntity paymentDetailEntity = new ProjectCostPaymentDetailEntity();
            paymentDetailEntity.setId(StringUtil.buildUUID());
            paymentDetailEntity.setFee(new BigDecimal(result));
            paymentDetailEntity.setPointDetailId(costDetailEntity.getId());
            paymentDetailEntity.setProjectId(costDetailEntity.getProjectId());
            this.projectCostPaymentDetailDao.insert(paymentDetailEntity);
            ...
            //3.处理我的任务
            myTask.setHandlerId(companyUserList.get(0).getId());
            this.finishMyTask(myTask);

            //推送消息给经营负责人和企业负责人，到款已经收到
            this.sendMessageToOperaterAndOrgAdminForTaskType(myTask,accountId);
    ...
    private ResponseBean handleType10( MyTaskEntity myTask,String result,String status,String accountId,String paidDate) throws Exception {
        ...
        detailDTO.setCurrentCompanyUserId(companyUserList.get(0).getId());
        detailDTO.setFee(new BigDecimal(result));
        ResponseBean ajaxMessage = this.projectCostService.saveCostPaymentDetail(detailDTO);
        ...
        //推送消息给经营负责人和企业负责人，到款已经收到
        Map<String,Object> data = ajaxMessage.getData();
        //重新设置了targetId，因为合同回款到款的任务的targetId为发起收款的节点，
        // 但是，在推送消息给企业负责人和经营负责人的时候，需要推送的是收款明细的信息，所以设置了返回的paymentDetailId到myTask中
        myTask.setTargetId((String)data.get("paymentDetailId"));//
        myTask.setParam1((String)data.get("pointId"));
        this.sendMessageToOperaterAndOrgAdminForTaskType(myTask,accountId);
        return ResponseBean.responseSuccess("操作成功");
     ...
     * @return:
     */
    private ResponseBean handleType12( MyTaskEntity myTask,String status,String accountId) throws Exception {

        CompanyRelationAuditEntity entity = this.companyRelationAuditDao.selectById(myTask.getTargetId());
        CompanyRelationAuditDTO dto = new CompanyRelationAuditDTO();
        dto.setId(entity.getId());
        dto.setAuditStatus(entity.getAuditStatus());
        dto.setOrgPid(entity.getOrgPid());
        dto.setOrgId(entity.getOrgId());
        dto.setRelationType(entity.getRelationType());
        dto.setType(entity.getType());
        dto.setAuditStatus(status);
        dto.setAccountId(accountId);
        AjaxMessage ajaxMessage = companyRelationAuditService.processingApplicationOrInvitation(dto);

        //处理我的任务
        myTask.setStatus("1");
        this.myTaskDao.updateById(myTask);
        return ResponseBean.responseSuccess((String) ajaxMessage.getInfo());
    }

    private void sendTaskToPartBFinacial(MyTaskEntity myTask) throws Exception{
        ProjectEntity projectEntity = this.projectDao.selectById(myTask.getProjectId());
        if(projectEntity!=null){
            if(!StringUtil.isNullOrEmpty(projectEntity.getCompanyBid())){
                Map<String, Object> map = new HashMap<String, Object>();
                map.put("permissionId", "49");//具有财务收款的负责人
                map.put("companyId", projectEntity.getCompanyBid());

                List<CompanyUserTableDTO> companyUserList = this.companyUserDao.getCompanyUserByPermissionId(map);
                MyTaskEntity entity = this.taskEntity8(myTask.getTargetId());
                entity.setTaskType(SystemParameters.TECHNICAL_REVIEW_FEE_PAYMENT_CONFIRM);
                entity.setCompanyId(projectEntity.getCompanyBid());
                entity.setTaskType(SystemParameters.TECHNICAL_REVIEW_FEE_PAYMENT_CONFIRM);
                ProjectMemberEntity managerEntity = this.projectMemberService.getOperatorManager(myTask.getProjectId(),entity.getCompanyId());
                boolean isSendManager = true;
                if(!CollectionUtils.isEmpty(companyUserList)){
                    for(CompanyUserTableDTO dto :companyUserList){
                        entity.setId(StringUtil.buildUUID());
                        entity.setHandlerId(dto.getId());
                        this.myTaskDao.insert(entity);
                        if(managerEntity!=null && managerEntity.getCompanyUserId().equals(dto.getId())){
                            isSendManager = false;
                        }
                        this.messageService.sendMessage(this.getMessage(entity));
                    }
                }else {
                    //保存一条空的数据
                    entity.setId(StringUtil.buildUUID());
                ...
                if(isSendManager){
                    if(managerEntity!=null) {
                        entity.setHandlerId(managerEntity.getCompanyUserId());
                        this.messageService.sendMessage(this.getMessage(entity, SystemParameters.MESSAGE_TYPE_12));
                    }
                }
            }
        }
    ...
    private void sendTaskToCooperativeFinacial(MyTaskEntity myTask,ProjectCostPointDetailEntity costDetailEntity) throws Exception{
        ProjectCostPointEntity pointEntity = this.projectCostPointDao.selectById(costDetailEntity.getPointId());
        String taskId = pointEntity.getTaskId();
        if(StringUtil.isNullOrEmpty(taskId)){
            ProjectCostPointEntity pointParent = this.projectCostPointDao.selectById(pointEntity.getPid());
            if(pointParent!=null){
                taskId = pointParent.getTaskId();
            }
        }
        if(!StringUtil.isNullOrEmpty(taskId))
        {
            ProjectTaskRelationEntity relationEntity = this.projectTaskRelationDao.getProjectTaskRelationByTaskId(taskId);
            if(relationEntity!=null){
                Map<String, Object> map = new HashMap<String, Object>();
                map.put("permissionId", "49");//具有财务收款的负责人
                map.put("companyId", relationEntity.getToCompanyId());
                List<CompanyUserTableDTO> companyUserList = this.companyUserDao.getCompanyUserByPermissionId(map);

                MyTaskEntity entity = this.taskEntity8(myTask.getTargetId());
                entity.setTaskType(SystemParameters.COOPERATIVE_DESIGN_FEE_PAYMENT_CONFIRM);
                entity.setCompanyId(relationEntity.getToCompanyId());
                ProjectMemberEntity managerEntity = this.projectMemberService.getOperatorManager(myTask.getProjectId(),entity.getCompanyId());
                boolean isSendManager = true;
                if(!CollectionUtils.isEmpty(companyUserList)){
                    for(CompanyUserTableDTO dto :companyUserList){
                ...
                if(isSendManager){
                    if(managerEntity!=null){
                        entity.setHandlerId(managerEntity.getCompanyUserId());
                        this.messageService.sendMessage(this.getMessage(entity,SystemParameters.MESSAGE_TYPE_15));
                    }
    ...
    private ResponseBean handleType22( MyTaskEntity myTask,String status,String accountId) throws Exception{
        ...
        myTask.setStatus("1");
        this.myTaskDao.updateById(myTask);
        this.projectTaskService.handleProjectTaskCompleteDate(myTask.getTargetId(),null);
        return ResponseBean.responseSuccess("操作成功");
    }
    ...
    public ResponseBean getMyTaskDetail(String id,String accountId) throws Exception {
        ...
        if(myTaskEntity!=null){
            ...
            if(type==4 || type==5 || type==6 || type==7 || type==10 || type==20 || type==21){//合同回款收款，合作设计费付款，技术审查费付款，其他费用收款，其他费用付款
                r =  this.projectCostService.getProjectCostPointDetailForMyTask(null,myTaskEntity.getTargetId(),myTaskEntity.getTaskType(),myTaskEntity.getCompanyId());
            }
            if(type>15 && type<20){//合作设计费，技术审查费  财务付款，到款，

                r =  this.projectCostService.getProjectCostPointDetailForMyTask(myTaskEntity.getTargetId(),null,myTaskEntity.getTaskType(),myTaskEntity.getCompanyId());
            }
            if(type==11){//报销
    ...
    private MessageEntity getMessage(MyTaskEntity task,int messageType) {
        CompanyUserEntity userEntity = this.companyUserDao.selectById(task.getHandlerId());
        MessageEntity messageEntity = new MessageEntity();
        messageEntity.setProjectId(task.getProjectId());
        messageEntity.setCompanyId(task.getCompanyId());
     ...
     * @return:
     */
    private void sendMessageToOperaterAndOrgAdminForTaskType(MyTaskEntity task,String accountId) throws Exception{
        ProjectMemberEntity managerEntity = this.projectMemberService.getOperatorManager(task.getProjectId(), task.getCompanyId());
        Map<String, Object> map = new HashMap<String, Object>();
        map.put("permissionId", "50");//企业负责人权限id
        ...
        if (managerEntity != null) {
            CompanyUserEntity userEntity = this.companyUserDao.selectById(managerEntity.getCompanyUserId());
            if (!userEntity.getUserId().equals(accountId)) {
                if (!userIdList.contains(userEntity.getUserId())) {
                    userIdList.add(userEntity.getUserId());
                }
            }
            if (!CollectionUtils.isEmpty(companyUserList) && !companyUserList.get(0).getUserId().equals(accountId)) {
                if (!userIdList.contains(companyUserList.get(0).getUserId())) {
                    userIdList.add(companyUserList.get(0).getUserId());
                }
            }
    ...
    private int getMyTaskState(MyTaskEntity entity) throws Exception {
        ...
        switch (taskType) {
            case 3:
                return this.projectTaskDao.getTaskState(entity.getParam1());
            case 12:
            case 13:
            case 22:
                return this.projectTaskDao.getTaskState(entity.getTargetId());
            default:
                return 0;
        }
    }
