...
public interface ProjectCostConst {
    ...
    List<Map<String,String>> COST_TYPE_RECEIVE_LIST = Lists.newArrayList(
            MapUtil.map("type",FEE_TYPE_CONTRACT+"","typeName","合同回款"),
            MapUtil.map("type",FEE_TYPE_TECHNICAL+"","typeName","技术审查费"),
            MapUtil.map("type",FEE_TYPE_COOPERATE+"","typeName","合作设计费"),
            MapUtil.map("type",FEE_TYPE_IN+"","typeName","其他收款")
            );

    List<Map<String,String>> COST_TYPE_PAY_LIST = Lists.newArrayList(
            MapUtil.map("type",FEE_TYPE_TECHNICAL+"","typeName","技术审查费"),
            MapUtil.map("type",FEE_TYPE_COOPERATE+"","typeName","合作设计费"),
            MapUtil.map("type",FEE_TYPE_OUT+"","typeName","其他付款")
    );
