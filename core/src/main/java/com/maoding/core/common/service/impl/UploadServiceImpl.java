...
public class UploadServiceImpl implements UploadService{
	...
	@Override
	public String uploadPdfImgWithByFdfs(BufferedImage image, String[] path) throws Exception {
		TxtFileUtil txtfileUtil = new TxtFileUtil();
		File file = new File(path[3]);
		if(!file.exists())
		{
		...
		File tmpFile1 = new File(path[4]);
		try{
			ImageIO.write(image, "jpg", tmpFile1);
		}catch (Exception e){
			e.printStackTrace();
		}
		//上传到fastdfs
		String masterFilePath = "";
		//上传原图
		masterFilePath = this.uploadFileByFdfs(path[4]);
		if ("".equals(masterFilePath)) {
			log.error("上传图片失败!");
			return "";
		}else {
			//删除本地文件
			txtfileUtil.deleteFile(path[4]);//删除本地原图
		}
		return masterFilePath;
	}
	 ...
	 */
	@Override
	public List<String> pdf2Img(MultipartFile imgObj) throws Exception {

		List<String> imgList = new ArrayList<String>();
			try {
				PDDocument doc = PDDocument.load(imgObj.getInputStream());
				PDFRenderer renderer = new PDFRenderer(doc);
			int pageCount = doc.getNumberOfPages();
			for(int i=0;i<pageCount;i++){
				BufferedImage image = renderer.renderImageWithDPI(i,60); // Windows native DPI
				//BufferedImage srcImage = resize(image, 540, 700);//产生缩略图
				String[] path = this.getWorksResourceUrl("image", pdfimg);
				String filePath = this.uploadPdfImgWithByFdfs(image, path);
