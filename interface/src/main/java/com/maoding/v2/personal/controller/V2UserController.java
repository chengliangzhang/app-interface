package com.maoding.v2.personal.controller;
...
/**
 * 深圳市设计同道技术有限公司
 * 类    名 : V2UserController
 * 作    者 : ChenZhujie
 * 日    期 : 2016/12/23
 */
@Controller
@RequestMapping("/v2/user")
public class V2UserController extends BaseWSController {
    ...
    public ResponseBean getUserInfo(@RequestBody Map<String, String> paraMap) throws Exception {
        ...
        UserInfoDTO userInfoDTO = this.userService.getUserInfo(id);
        if (userInfoDTO == null) {
            return ResponseBean.responseError("请求数据异常");
        }
    ...
    public ResponseBean invitationLink(@RequestBody Map<String, String> map) {
        return responseSuccess()
                .addData("link", serverUrl + "/iWork/sys/shareInvitation/" + map.get("companyId") + "/" + map.get("accountId"));
    }
     ...
     * 日    期 : 2016/12/23
     */
    @RequestMapping("/userCardLink")
    @ResponseBody
    @AuthorityCheckable
    public ResponseBean userCardLink(@RequestBody Map<String, String> map) {
        return responseSuccess()
                .addData("link", serverUrl + "/iWork/sys/sharePersonalInfo/" + map.get("companyId") + "/" + map.get("accountId"));
    }
    ...
    @ResponseBody
    @AuthorityCheckable
    public ResponseBean changePassword(@RequestBody AccountDTO accountDto) throws Exception {
        ...
        if (loginUser.getPassword().equals(accountDto.getOldPassword()))//移动端返回的密码直接是加密过的
        {
            ...
            if (i == 1) {
                imService.updateImAccount(userId, accountDto.getPassword());
    ...
    public ResponseBean bindEmail(@RequestBody AccountDTO dto) throws Exception {
        ...
        if (checkCode(sessionid + dto.getEmail(), code)) {
            ...
            if (email == null || !(StringUtil.isEmail(email)) || null != accountService.getAccountByCellphoneOrEmail(email)) {
                ...
                if (i == 1) {
                    return ResponseBean.responseSuccess("绑定邮箱成功");
                } else {
...
//    }
     ...
     * 日    期 : 2016/12/23
     */
    @RequestMapping("/v2UploadAttach")
    @ResponseBody
    public ResponseBean v2UploadAttach(UploadAttachDTO attachDTO) {
        try {
            Map<String, Object> param = new HashMap<String, Object>();
            param.put("type", attachDTO.getAttachType());
            param.put("userId", attachDTO.getAccountId());
            ...
            //此标识是用来标示app端上传头像头像
            param.put("cutFlg", "1");
            Object object = userAttachService.v2SaveUserAttach(attachDTO.getUploadFile(), param);
            if (null == object) {
                return ResponseBean.responseError("上传头像失败");
            }
            AjaxMessage ajaxMessage = (AjaxMessage) object;
     ...
     * 日    期 : 2016/12/23
     */
    @RequestMapping("v2SaveOrupdateUser")
    @ResponseBody
    @AuthorityCheckable
    public ResponseBean v2SaveOrupdateUser(@RequestBody AccountDTO dto) throws Exception {
        return returnResponseBean(userService.v2SaveOrUpdateUser(dto));
    }
    ...
    @ResponseBody
    public ResponseBean getMajor() throws Exception {
        return ResponseBean.responseSuccess().addData("majorList", this.dataDictionaryService.getSubDataByCode(SystemParameters.USER_MAJOR));
    }
